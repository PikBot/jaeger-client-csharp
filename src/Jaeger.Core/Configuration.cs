    ...
    public class Configuration
    {
        ...
        public class SamplerConfiguration
        {
            ...
            internal ISampler CreateSampler(string serviceName, IMetrics metrics)
            {
                ...
                if (string.Equals(samplerType, ConstSampler.Type, StringComparison.Ordinal))
                {
                    return new ConstSampler(samplerParam != 0);
                }

                if (string.Equals(samplerType, ProbabilisticSampler.Type, StringComparison.Ordinal))
                {
                    return new ProbabilisticSampler(samplerParam);
                }

                if (string.Equals(samplerType, RateLimitingSampler.Type, StringComparison.Ordinal))
                {
                    return new RateLimitingSampler(samplerParam);
                }

                if (string.Equals(samplerType, RemoteControlledSampler.Type, StringComparison.Ordinal))
                {
